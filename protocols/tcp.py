...
class TCPConnection:
    def __init__(self, env, src, dst, src_nic_speed, dst_nic_speed, routers, cca='reno'):
        ...
        # RFC 6298: RTO calculation parameters
        self.rto = 1  # Initial RTO (RFC 6298: 1 second)
        self.rto_min = 1  # Minimum RTO (RFC 6298: 1 second)
        self.rto_max = 60  # Maximum RTO (practical upper bound)
        self.rto_backoff = 1  # Exponential backoff multiplier
        self.G = 1  # Clock granularity (RFC 6298: G, set to 1s for simulation)
        self.segments = deque()
    ...
    def check_timeout(self, seq):
        def timeout_check():
            ...
            if seq >= self.snd_una and self.state != 'CLOSED':
                print(f"TCP {self.connection_id} at {self.env.now}: Timeout detected for seq {seq} (RTO={self.rto}) [RFC 6298]")
                self.cca.on_loss(is_timeout=True)
                # RFC 6298: Exponential backoff: double RTO, clamp to max
                self.rto_backoff = min(self.rto_backoff * 2, self.rto_max // self.rto_min)
                self.rto = min(self.rto * 2, self.rto_max)
                print(f"TCP {self.connection_id}: RTO backed off to {self.rto} (multiplier {self.rto_backoff}) [RFC 6298]")
    ...
    def route_segment(self, segment):
        ...
        if isinstance(packet.source, Processor):
            ...
            print(f"Source is DTN {packet.source.dtn_id}, Destination Processor {packet.destination.processor_id}, using DTN's edge router {packet.source.edge_router.router_id}")
            router = packet.source.edge_router  # Use DTN's edge router to force bottleneck routing     
        else:
        ...
        if router:
            print(f"Routing segment {segment.seq} with flag {segment.flags} for job {segment.job_id} via Router {router.router_id}")
            self.env.process(router.receive_packet(packet))
        else:
    ...
    def _wait_for_acks(self, job_id, total_segments):
        print(f"TCP {self.connection_id} at {self.env.now}: _wait_for_acks started for job {job_id}, waiting for snd_una {self.snd_una} to reach total_segments {total_segments}")
        while self.snd_una < total_segments and self.state != 'CLOSED':
            print(f"TCP {self.connection_id} at {self.env.now}: Waiting for ACKs, snd_una={self.snd_una}, total_segments={total_segments}")
            yield self.env.timeout(1)
        print(f"TCP {self.connection_id} at {self.env.now}: All segments acknowledged! Sending FIN for job {job_id}")
    ...
    def handle_ack(self, segment):
        ...
        if segment.seq == self.num_total_segments:
            ...
            print(f"TCP {self.connection_id} at {self.env.now}: Calling _send_data for job {segment.job_id}")
    ...
    def update_rtt(self, ack):
        if ack-1 in self.send_times:
            ...
            if self.srtt is None:
                # RFC 6298: First measurement
                self.srtt = rtt
                self.rttvar = rtt / 2
            else:
                # RFC 6298: Update SRTT and RTTVAR
                self.rttvar = (1 - 0.25) * self.rttvar + 0.25 * abs(self.srtt - rtt)
                self.srtt = (1 - 0.125) * self.srtt + 0.125 * rtt
            # RFC 6298: RTO = SRTT + max(G, 4*RTTVAR), clamp to min/max
            new_rto = self.srtt + max(self.G, 4 * self.rttvar)
            self.rto = min(max(new_rto, self.rto_min), self.rto_max)
            self.rto_backoff = 1  # Reset backoff on successful ACK
            ...
            print(f"TCP {self.connection_id} at {self.env.now}: Updated RTT to {rtt}, srtt to {self.srtt}, rttvar to {self.rttvar}, rto to {self.rto} [RFC 6298]")
    ...
    def _send_data(self, job_id):
        print(f"TCP {self.connection_id} at {self.env.now}: _send_data called for job {job_id}, snd_nxt={self.snd_nxt}, num_total_segments={self.num_total_segments}")
        ...
        if self.state == 'RECOVERY':
            ...
            while self.snd_nxt < self.num_total_segments:
                print(f"TCP {self.connection_id} at {self.env.now}: Open - sending data, snd_nxt={self.snd_nxt}, segments_to_send={segments_to_send}, num_full_segments={self.num_full_segments}") 
                if self.snd_nxt < segments_to_send:
                    if self.snd_nxt < self.num_full_segments:
                        ...
                        print(f"TCP {self.connection_id} at {self.env.now}: Sending LAST DATA segment {self.snd_nxt} with {self.remaining_Mbytes} MB for job {job_id}")
                        yield self.env.process(self.send_segment(job_id=job_id, seq=self.snd_nxt, flags='DATA', data=self.remaining_Mbytes))
                        print(f"TCP {self.connection_id} at {self.env.now}: Starting _wait_for_acks process for job {job_id}, total_segments={self.num_total_segments}")
                    ...
                    print(f"TCP {self.connection_id} at {self.env.now}: Breaking - snd_nxt={self.snd_nxt} >= segments_to_send={segments_to_send}")
                    break
            
            # Check if we've sent all segments and need to start waiting for ACKs
            if self.snd_nxt >= self.num_total_segments:
                print(f"TCP {self.connection_id} at {self.env.now}: All segments sent! Starting _wait_for_acks process for job {job_id}, total_segments={self.num_total_segments}")
                self.env.process(self._wait_for_acks(job_id, self.num_total_segments))
