...
class Cubic(CongestionControl):
    ...
    def on_loss(self, is_timeout=False):
        # RFC 8312 Fast Convergence: If cwnd < last_max_cwnd, set last_max_cwnd = (last_max_cwnd + cwnd) / 2
        if self.connection.cwnd < self.last_max_cwnd:
            self.last_max_cwnd = (self.last_max_cwnd + self.connection.cwnd) / 2
            print(f"Cubic CCA: Fast convergence applied, new last_max_cwnd = {self.last_max_cwnd}")
        if is_timeout:
