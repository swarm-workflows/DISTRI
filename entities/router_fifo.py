...
def handle_fifo(router, packet_info):
    ...
    if hasattr(packet, 'segment_flag') and packet.segment_flag == 'ACK':
        print(f"Router {router.router_id} at {router.env.now}: Processing ACK packet immediately")
        router.env.process(router.send_packet(packet))
        return
    
    # Normal queueing for other packets
...
class RouterFifo:
    ...
    def send_packet(self, packet):
        ...
        if destination in self.routing_table:
            ...
            # Forward to next hop if it's a router, otherwise forward to destination directly
            if next_hop != destination:
                # Routing via another router
            ...
            yield self.env.timeout(0)  # Make it a generator for SimPy
