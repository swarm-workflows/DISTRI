...
class RouterFq:
    ...
    def receive_packet(self, packet):
        ...
        # Immediately process ACK packets to prevent job completion issues
        if hasattr(packet, 'segment_flag') and packet.segment_flag == 'ACK':
            print(f"Router {self.router_id}: Processing ACK packet immediately")
        ...
        if destination in self.routing_table:
            ...
            # If destination has a direct queue, use it
            if destination in self.queue_ids:
                queue_id = self.queue_ids[destination]
                print(f"Router {self.router_id}: Enqueue packet to queue for destination {destination} via queue_id {queue_id}.")
                if len(self.queues[queue_id].items) < self.limit_per_queue:
                    self.queues[queue_id].put({'packet': packet, 'time_queued': self.env.now})
                    self.record_load(queue_id)
                else:
                    print(f"Router {self.router_id}: Queue for destination {destination} is full. Dropping packet.")
            else:
                # For via routes, forward immediately
                print(f"Router {self.router_id}: Routing packet to {destination} via {next_hop} (from routing table)")
                self.env.process(self.send_packet(packet))

        else:
            print(f"Router {self.router_id}: No route to destination {destination}. Dropping packet.")
        
        # Make this a generator for SimPy
        yield self.env.timeout(0)
