...
class Processor:
    ...
    def initiate_job(self, job):
        ...
        print(f"Processor {self.processor_id}: Waiting for data event for job {job.id}")
        yield data_event  # Wait here until the data arrives
        print(f"Processor {self.processor_id}: Data event triggered for job {job.id}")
        ...
        # Process job immediately instead of using ready_jobs queue
        print(f"Processor {self.processor_id}: Starting immediate processing of job {job.id}")
        yield self.env.process(self.process_job(job))
        print(f"Processor {self.processor_id}: Completed immediate processing of job {job.id}")
    ...
    def process_job(self, job):
        """ Processes the job. """
        print(f"Processor {self.processor_id}: process_job called for job {job.id}")
        job.start_time = self.env.now
        process_time = job.computation / self.compute_speed
        print(f"Processor {self.processor_id}: Job {job.id} needs {process_time} time units (computation={job.computation}, speed={self.compute_speed})")
        ...
        print(f"Processor {self.processor_id}: Job {job.id} marked complete in resource pool")
    ...
    def receive_data_sent_flag(self, job_id):
        ...
        print(f"Processor {self.processor_id}: receive_data_sent_flag called for job {job_id}")
        for job, event in self.waiting_jobs:
            if job.id == job_id:
                print(f"Processor {self.processor_id}: Found waiting job {job_id}, calling event.succeed()")
                event.succeed()
                self.waiting_jobs.remove((job, event))
                print(f"Processor {self.processor_id}: Job {job_id} marked as complete")
                break
        else:
            print(f"Processor {self.processor_id}: Job {job_id} not found in waiting_jobs")
    ...
    def receive_segment(self, segment):
        ...
        if segment.flags == 'FIN':
            ...
            # Check if we have received all expected data for this job
            connection = self.connections[segment.job_id][segment.connection_id]
            print(f"Processor {self.processor_id}: Checking completion for job {segment.job_id}, expected_seq={expected_seq}")
            if hasattr(connection, 'num_total_segments'):
                print(f"Processor {self.processor_id}: connection.num_total_segments={connection.num_total_segments}")
                
                # Check if we have received all segments (alternative completion check)
                received_segments = self.received_data[segment.job_id][segment.connection_id]
                received_seq_numbers = [seg.seq for seg in received_segments]
                max_seq = max(received_seq_numbers) if received_seq_numbers else -1
                print(f"Processor {self.processor_id}: received_seq_numbers={received_seq_numbers}, max_seq={max_seq}")
                
                # Check if all segments received
                if expected_seq >= connection.num_total_segments or (max_seq + 1) >= connection.num_total_segments:
                    print(f"Processor {self.processor_id}: All data received for job {segment.job_id} (expected_seq={expected_seq}, max_seq+1={max_seq+1}, num_total_segments={connection.num_total_segments}, received_segments={len(received_seq_numbers)})")
                    self.receive_data_sent_flag(segment.job_id)
                    # Close the connection
                    if connection.state != 'CLOSED':
                        connection.close()
            else:
                print(f"Processor {self.processor_id}: connection does not have num_total_segments attribute")
