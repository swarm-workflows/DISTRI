...
def job_generator(env, resource_pool, num_categories, category_profiles, seed=None):
    ...
    while True:
        ...
        # Use random data volumes as originally intended
        ...
        print(f"Generated and added job {job.id} in category {job.category} to ResourcePool {resource_pool.resource_pool_id} with comp {computation} and vol {data_volume} MB. Total jobs now: {len(resource_pool.common_pool[job.category].items)}")
