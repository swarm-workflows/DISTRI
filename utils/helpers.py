...
def task_generator(env, resource_pool, num_categories, category_profiles, seed=None):
    ...
    while True:
        ...
        task_id += 1
        yield env.timeout(random.uniform(0.1, 1.0))  # New tasks are generated every 0.1 - 3.0 time units
...
def create_network(env, num_categories, nic_speed, delay, aqm):
    # Create a router for each category with a unique router_id
    if aqm == 'fifo':
        routers = [RouterFifo(env, i, nic_speed, delay) for i in range(num_categories)]
    elif aqm == 'fq':
        routers = [RouterFq(env, i, nic_speed, delay) for i in range(num_categories)]
    else:
        raise ValueError('AQM Error: Unsupported AQM input "{}"'.format(aqm))
    return routers


def connect_processors_to_routers(env, num_categories, processors_per_category, resource_pool, dtn, processor_task_lookup_time, pheromone_map, routers, processor_category_profiles):
    processors = []
    ...
    for category in range(num_categories):
        for index in range(processors_per_category):
            ...
            processor = Processor(env, processor_id, category, resource_pool, routers[category], dtn, profile['compute_speed'], profile['nic_speed'], processor_task_lookup_time, pheromone_map, profile['max_concurrent_tasks'])
